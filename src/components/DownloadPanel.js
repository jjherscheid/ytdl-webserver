...
import { post, get } from '../javascripts/helpers'
...
class DownloadPanel extends Component {
  ...
  handleSubmit(e) {
    ...
    const pswInput = document.querySelector('.downloadForm__psw')
    const code = pswInput.value
    ...
    post('/download', `url=${url}&code=${code}`).then(newVideo => {
